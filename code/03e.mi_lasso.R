...
here::i_am("01.code/03e.mi_lasso.R")
library(here)
library(glue)
library(data.table)
library(tidymodels)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## format data from lists to one single tibble --------

# Recipes
# rc <-
#   tibble(rc = nest_recipe) %>%
#   unnest_longer(rc, indices_to = "imp") %>%
#   hoist(rc, recipe = "recipe") %>%
#   select(-c(imp, rc))
#
# # parameters
# pr <-
#   tibble(pr = params) %>%
#   unnest_longer(pr, indices_to = "imp") %>%
#   unnest_wider(pr) %>%
#   select(-imp)
#
#
# # testing and training datasets
# var_sample <- names(good_LC)
#
# yv <- tibble(yv         = good_LC,
#              var_sample = var_sample) %>%
#   mutate(y = gsub("(.+)-(.+)", "\\1", var_sample),
#          sample = gsub("(.+)-(.+)", "\\2", var_sample))
#
# # put together all information
# tb_all <-
#   yv %>%
#   unnest_longer(yv, indices_to = "imp")  %>%
#   unnest_wider(yv) %>%
#   bind_cols(rc, pr) %>%
#   # Change var order
#   relocate(training, testing, recipe, y, penalty, mixture)
#
# # create a tibble only for inputs of run_nowcast function
# tb_input <- tb_all %>%
#   select(training, testing, recipe, y, penalty, mixture)
#
# # get ids
# tb_id <- tb_all %>%
#   select(y, sample, imp, var_sample)
#
#
# # get back wbcode
# # wbcodes <-
...
#         ts_id = "ts_id",
#         tr_id = "tr_id") %>%
#   hoist(ts_id, wbcode_ts = "wbcode") %>%
#   hoist(tr_id, wbcode_tr = "wbcode") %>%
#   select(wbcode_ts, wbcode_tr)
#
# wbcodes <-
#   map2(.x = wbcode_ts_tr$wbcode_ts,
#        .y = wbcode_ts_tr$wbcode_tr,
#        .f = ~{c(.x, .y)}) %>%
#   tibble(wbcode = .)
...
if (run_l_out) {
  ...
  # tvec <- 1:2
  # wbcodes    <- wbcodes[tvec, ]
  # tb_input   <- tb_input[tvec, ]
  # tb_id      <- tb_id[tvec, ]
  # var_sample <- var_sample[tvec ]

  l_out <- pmap(tb_input, run_nowcast)
  readr::write_rds(l_out, here("02.inputdata/mi/l_out.rds"))
} else {
  l_out <- readr::read_rds(here("02.inputdata/mi/l_out.rds"))
}


# Run post-lasso estimations
tb_pl_input <- # Post-lasso input
  tibble(res = l_out) %>%
  unnest_wider(res) %>%
  bind_cols(tb_input) %>%
  select(training, testing, y, cft, recipe)

# Run post-lasso model
if (run_pl_out) {
  pl_out <- pmap(tb_pl_input, pl_nowcast)
  readr::write_rds(l_out, here("02.inputdata/mi/pl_out.rds"))
} else {
  pl_out <- readr::read_rds(here("02.inputdata/mi/pl_out.rds"))
}

# Bind wbcodes to each testing data
df <-
  wbcodes %>%
  bind_cols(tibble(res = l_out)) %>%
  unnest_wider(res) %>%
  # hoist(est_testing, df = "df") %>%
  hoist(est_all, df = "df") %>%
  select(wbcode, df) %>%
  map2(.x = .$df,
       .y = .$wbcode,
...
pl_df <-
  wbcodes %>%
  bind_cols(tibble(res = pl_out)) %>%
  unnest_wider(res) %>%
  select(wbcode, df = est_all) %>%
  map2(.x = .$df,
       .y = .$wbcode,
       .f = ~ {
         tibble(wbcode = .y) %>%
...
agg_res <-
  ...
  tibble(pl_df = pl_df) %>%
  bind_cols(tb_id) %>%
  select(var_sample, df, pl_df) %>%
  nest(data = c(df, pl_df)) %>%
  select(data) %>%
  as.list() %>%
  pluck(1)
names(agg_res) <- good_yvars
...
# GEt average of imputed datasets
ld <- map(agg_res,
          ~{
            z <- .x$df  # extract the list inside the tibble
            a <- .x$pl_df  # extract the list inside the tibble

            # create var with imputation number and append all imputations
            df_app <- imap_dfr(.x = z,
                               .f = ~{
                                 .x %>%
                                   mutate(imp = .y)  # imputation number
                               })

            pl_df_app <- imap_dfr(.x = a,
                                  .f = ~{
                                    .x %>%
                                      mutate(imp = .y)  # imputation number
                                  })
            ...
            l_ave <-
              df_app %>%
              group_by(wbcode, year, datatype) %>%
              summarise(.pred_l = mean(.pred, na.rm = TRUE),
                        .groups = 'drop')

            pl_ave <-
              pl_df_app %>%
              group_by(wbcode, year, datatype) %>%
              summarise(.pred_pl = mean(.pred, na.rm = TRUE),
                        .groups = 'drop')

            ave <- joyn::merge(l_ave, pl_ave,
                        by = c("wbcode", "year", "datatype"),
                        match_type = "1:1",
                        reportvar = FALSE,
                        verbose = FALSE)
...
pred_y <-
  map2(.x = ld,
       ...
       .f = ~{
         ...
         # Create name
         y    <- gsub("\\-", "_", .y)
         y_l  <- paste0(y, "_pred_l")
         y_pl <- paste0(y, "_pred_pl")

         # Rename and get only the predicted vector
         .x %>%
           rename({{y_l}} := .pred_l,
                  {{y_pl}} := .pred_pl)
