...
# Loading data
data = read_dta("02.inputdata/FinalInputData_lag.dta")
...
for (yvar in c(
  ...
  for (sample in c("al", "co")) {
    ...
    ggsave(
      paste(
        '05.figures/varimp/varimp_cirl_',
        noquote(yvar),
        '_',
        noquote(sample),
        '.jpg',
    ...
    predname = paste(noquote(yvar), '_cirl_', noquote(sample), '_dire', sep =
                       "")
    ...
    write_dta(data, "03.intermediatedata/Predictions/ciforest_lag.dta")
...
data <- data %>%
  group_by(code, datatype) %>%
  mutate(Y_l_head_cirl_al_grow = laghead + Y_c_head_cirl_al_dire * (year -
                                                                      lagyear)) %>%
  mutate(Y_l_head_cirl_co_grow = laghead + Y_c_head_cirl_co_dire * (year -
                                                                      lagyear)) %>%
  mutate(Y_l_mean_cirl_al_grow = lagmean * (Y_g_mean_cirl_al_dire + 1) ^
           (year - lagyear)) %>%
  mutate(Y_l_mean_cirl_co_grow = lagmean * (Y_g_mean_cirl_co_dire + 1) ^
           (year - lagyear)) %>%
  mutate(Y_l_medi_cirl_al_grow = lagmedi * (Y_g_medi_cirl_al_dire + 1) ^
           (year - lagyear)) %>%
  mutate(Y_l_medi_cirl_co_grow = lagmedi * (Y_g_medi_cirl_co_dire + 1) ^
           (year - lagyear)) %>%
  mutate(Y_l_gini_cirl_al_grow = laggini * (Y_g_gini_cirl_al_dire + 1) ^
           (year - lagyear)) %>%
  mutate(Y_l_gini_cirl_co_grow = laggini * (Y_g_gini_cirl_co_dire + 1) ^
           (year - lagyear))
...
for (reps in seq(1, 21)) {
  data <- data %>%
    group_by(code, datatype) %>%
    mutate(
      Y_l_head_cirl_al_grow = ifelse(
        is.na(Y_l_head_cirl_al_grow),
        lag(Y_l_head_cirl_al_grow) + Y_c_head_cirl_al_dire * (year - lagyear),
        Y_l_head_cirl_al_grow
      )
    ) %>%
    mutate(
      Y_l_head_cirl_co_grow = ifelse(
        is.na(Y_l_head_cirl_co_grow),
        lag(Y_l_head_cirl_co_grow) + Y_c_head_cirl_co_dire * (year - lagyear),
        Y_l_head_cirl_co_grow
      )
    ) %>%
    mutate(Y_l_mean_cirl_al_grow = ifelse(
      is.na(Y_l_mean_cirl_al_grow),
      lag(Y_l_mean_cirl_al_grow) * (Y_g_mean_cirl_al_dire + 1) ^ (year - lagyear),
      Y_l_mean_cirl_al_grow
    )) %>%
    mutate(Y_l_mean_cirl_co_grow = ifelse(
      is.na(Y_l_mean_cirl_co_grow),
      lag(Y_l_mean_cirl_co_grow) * (Y_g_mean_cirl_co_dire + 1) ^ (year - lagyear),
      Y_l_mean_cirl_co_grow
    )) %>%
    mutate(Y_l_medi_cirl_al_grow = ifelse(
      is.na(Y_l_medi_cirl_al_grow),
      lag(Y_l_medi_cirl_al_grow) * (Y_g_medi_cirl_al_dire + 1) ^ (year - lagyear),
      Y_l_medi_cirl_al_grow
    )) %>%
    mutate(Y_l_medi_cirl_co_grow = ifelse(
      is.na(Y_l_medi_cirl_co_grow),
      lag(Y_l_medi_cirl_co_grow) * (Y_g_medi_cirl_co_dire + 1) ^ (year - lagyear),
      Y_l_medi_cirl_co_grow
    )) %>%
    mutate(Y_l_gini_cirl_al_grow = ifelse(
      is.na(Y_l_gini_cirl_al_grow),
      lag(Y_l_gini_cirl_al_grow) * (Y_g_gini_cirl_al_dire + 1) ^ (year - lagyear),
      Y_l_gini_cirl_al_grow
    )) %>%
    mutate(Y_l_gini_cirl_co_grow = ifelse(
      is.na(Y_l_gini_cirl_co_grow),
      lag(Y_l_gini_cirl_co_grow) * (Y_g_gini_cirl_co_dire + 1) ^ (year - lagyear),
      Y_l_gini_cirl_co_grow
    ))
}

# In case some growth predictions are outside of logical intervals
data$Y_l_head_cirl_al_grow[data$Y_l_head_cirl_al_grow > 100] = 100
data$Y_l_head_cirl_co_grow[data$Y_l_head_cirl_co_grow > 100] = 100
data$Y_l_gini_cirl_al_grow[data$Y_l_gini_cirl_al_grow > 70]  = 70
data$Y_l_gini_cirl_co_grow[data$Y_l_gini_cirl_co_grow > 70]  = 70
data$Y_l_head_cirl_al_grow[data$Y_l_head_cirl_al_grow < 0]   = 0
data$Y_l_head_cirl_co_grow[data$Y_l_head_cirl_co_grow < 0]   = 0
