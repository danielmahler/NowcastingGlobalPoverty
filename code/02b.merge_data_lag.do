...
***********************************
*** LEAD ALL VARIABLES ONE YEAR ***
***********************************
// This is the only difference between the 02a .do-file and the 02b .do-file:
gsort code -year
foreach var of varlist WDI* WEO* CLASS* PCN* NA* RS* {
...
foreach var of varlist PCN_l_pop NA* WEO* WDI* RS* {
...
replace sample_al_l = 0 if missing(sample_al_l)
replace sample_co_l = 0 if missing(sample_co_l)
replace sample_al_g = 0 if missing(sample_al_g)
replace sample_co_g = 0 if missing(sample_co_g)
...
***************************************************
// Removing an " ' " from a variable label, otherwise the code below doesn't work
lab var WDI_l_SHDYNAIDSFEZS "Womens share of population ages 15+ living with HIV (%)"

// Annualized growth variables
foreach var of varlist Y_l_mean Y_l_medi Y_l_gini PCN_l_pop NA* WEO* WDI* RS* {
...
compress
save "02.inputdata\FinalInputData_lag.dta", replace